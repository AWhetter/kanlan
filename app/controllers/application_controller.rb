class ApplicationController < ActionController::Base
  ...
  protect_from_forgery with: :exception
	...
	def configure_permitted_parameters
		devise_parameter_sanitizer.for(:sign_up) { |u| u.permit(:username, :password, :password_confirmation, :table, :seat, :remember_me) }
		devise_parameter_sanitizer.for(:sign_in) { |u| u.permit(:login, :username, :password, :remember_me) }
		devise_parameter_sanitizer.for(:account_update) { |u| u.permit(:username, :password, :password_confirmation, :table, :seat, :current_password) }
	end
